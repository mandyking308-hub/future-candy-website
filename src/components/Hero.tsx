...
const Hero = () => {
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden pt-32 md:pt-36">
      <div className="absolute inset-0 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${heroVisual})`, backgroundPosition: "center 30%", filter: "brightness(0.6) contrast(1.1)" }} role="img" aria-label="NeonCandy hero visual with warm neon glow and glossy pop studio styling" />
      ...
      <div className="absolute inset-0 ambient-glow opacity-30" style={{ background: "radial-gradient(circle at 50% 50%, hsl(var(--candy-pink) / 0.4), hsl(var(--candy-violet) / 0.3), transparent 70%)" }} />
      <div className="relative z-10 container mx-auto px-4 text-center py-12 md:py-0">
        <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold mb-4 md:mb-6 animate-fade-in">
          ...
          <span className="block text-2xl sm:text-3xl md:text-4xl mt-4 text-foreground/90 font-medium">Sweet Beats. Synthetic Dreams.</span>
        </h1>
        <p className="text-xl sm:text-2xl md:text-3xl lg:text-4xl mb-3 md:mb-4 text-foreground/95 animate-fade-in drop-shadow-lg" style={{ animationDelay: "0.2s" }}>AI Pop for the Future.</p>
        <p className="text-base sm:text-lg md:text-xl lg:text-2xl mb-8 md:mb-12 text-foreground/90 animate-fade-in max-w-2xl mx-auto drop-shadow-md leading-relaxed" style={{ animationDelay: "0.4s" }}>A digital-first pop studio building artists, songs, videos, and visual worlds with AI-powered creative direction.</p>
        <div className="flex flex-col sm:flex-row gap-3 md:gap-4 justify-center items-center animate-fade-in px-4" style={{ animationDelay: "0.6s" }}>
          <a href="#about" onClick={scrollToSection} className="w-full sm:w-auto"><Button size="lg" className="w-full sm:w-auto glow-pink gap-2 text-base md:text-lg px-6 md:px-8 py-5 md:py-6 bg-gradient-to-r from-candy-pink to-candy-violet hover:scale-105 transition-transform shadow-[0_0_30px_rgba(236,72,153,0.6)] font-semibold">Enter the World</Button></a>
