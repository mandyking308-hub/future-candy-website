...
const Artists = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setArtists(
        (artistData || []).map((a: any) => ({ ...a, song_count: counts[a.id] || 0 }))
      );
      setLoading(false);
    };
    load();
  ...
  const filteredArtists =
    activeFilter === "All" ? artists : artists.filter(a => a.personality === activeFilter);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {filteredArtists.map((artist, index) => {
              ...
              return (
                <Card
                  key={artist.id}
                  className="group relative overflow-hidden border-2 hover:scale-105 hover:shadow-[0_0_40px_rgba(236,72,153,0.4)] transition-all duration-300 animate-fade-in glass"
                  style={{
                    animationDelay: `${index * 0.2}s`,
                    borderColor: `hsl(var(--${color}) / 0.5)`,
                  }}
                >
                    ...
                    <div className="aspect-square relative bg-muted/30">
                      {artist.image_url ? (
                        <img
                          ...
                          alt={`${artist.name} — NeonCandy artist`}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    {artist.description && (
                      <p className="text-foreground/80 mb-3 text-sm line-clamp-2">
                        {artist.description}
                      </p>
                    )}
                    ...
                    <div className="flex items-center gap-3 text-sm flex-wrap">
                      <Link
                        ...
                        className="flex items-center gap-1.5 text-foreground hover:text-candy-pink transition-colors"
                      >
